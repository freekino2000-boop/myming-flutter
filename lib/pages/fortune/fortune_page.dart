      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const Center(child: Text('사주 운세 보기', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w900))),
            Center(child: Text('+₩5 적립', style: TextStyle(color: AppColors.primary.withValues(alpha: 0.8), fontSize: 12))),
            const SizedBox(height: 24),

            const _Label('생년월일 (8자리)'),
            ...
            Row(children: ['남성', '여성'].map((g) => GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: gender == g ? AppColors.primary : AppColors.border.withValues(alpha: 0.3)),
        ...
        children: [
                  ...
                  child: Container(
                    ...
                    child: Center(child: isFront
                        ? const Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                            ...
                            SizedBox(height: 12),
                            Text('탭해서 운세 확인', style: TextStyle(color: Colors.white60, fontSize: 12)),
                          ])
                        : Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                            Text(result.emoji, style: const TextStyle(fontSize: 56)),
                            const SizedBox(height: 8),
                            Text(result.grade, style: const TextStyle(color: AppColors.primary, fontSize: 18, fontWeight: FontWeight.w900)),
                          ])),
                  ),
                );
              },
          ...
          if (flipped && flipAnim.value > 0.9) ...[
            ...
            Container(
              ...
              decoration: BoxDecoration(color: const Color(0xFF1A1A2E), border: Border.all(color: AppColors.border.withValues(alpha: 0.3)), borderRadius: BorderRadius.circular(20)),
              child: Column(
                children: [
                  ...
                  Container(
                    ...
                    child: Row(children: [
                      ...
                      Expanded(child: Text('오늘의 행운 아이템: ${result.luckyItem}', style: TextStyle(color: AppColors.primary.withValues(alpha: 0.9), fontSize: 12, fontWeight: FontWeight.w700))),
                    ]),
                  ),
                ],
              ),
...
_FortuneResult _randomResult() {
  final rng = Random();
  final results = [
    const _FortuneResult(emoji: '☀️', grade: '대길 (大吉)', title: '오늘은 빛나는 하루!', content: '오늘은 당신에게 행운이 가득한 날입니다. 새로운 도전을 두려워하지 마세요. 재물운이 강하게 흐르며, 마이밍으로 절감한 금액이 배로 돌아올 수 있습니다.', categories: {'재물운': 90, '건강운': 85, '연애운': 78, '직장운': 88}, luckyItem: '🟡 황금색 아이템 or 지갑'),
    const _FortuneResult(emoji: '🌤', grade: '길 (吉)', title: '좋은 기운이 흐르는 날', content: '작은 행운들이 겹치는 날입니다. 오늘 지출을 줄이면 더 큰 보상으로 돌아옵니다. 꾸준히 걷고 미션을 완료하면 월세 절감이 순조롭습니다.', categories: {'재물운': 75, '건강운': 80, '연애운': 68, '직장운': 72}, luckyItem: '💙 파란색 물건 or 물 한 잔'),
    const _FortuneResult(emoji: '☁️', grade: '보통', title: '평온한 흐름의 하루', content: '오늘은 평범하지만 안정적인 하루입니다. 큰 결정은 내일로 미루는 것이 좋습니다. 만보기를 꾸준히 채우면 작은 행운이 따를 것입니다.', categories: {'재물운': 60, '건강운': 65, '연애운': 55, '직장운': 62}, luckyItem: '🟢 초록색 식물 or 산책'),
    const _FortuneResult(emoji: '🌙', grade: '중길 (中吉)', title: '조용한 기운의 날', content: '지금은 때를 기다리는 시간입니다. 무리한 지출을 피하고 적립금을 쌓는 데 집중하세요. 꾸준함이 최고의 전략입니다.', categories: {'재물운': 68, '건강운': 72, '연애운': 60, '직장운': 70}, luckyItem: '⚪ 흰색 또는 은색 아이템'),
  ];
